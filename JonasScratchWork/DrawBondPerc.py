...
def draw_square_bonds(bond_bools, groups, graph_params=(4, 20, 4)):
    ...
    backup_colors = list(mcolors.CSS4_COLORS)[16:]
    np.random.shuffle(backup_colors)
    color_list = list(mcolors.TABLEAU_COLORS) + backup_colors
    ...
    available_groups = [num for num in range(len(color_list))]
    ...
    bond_color = [color_list[int(num)] for num in reordered_groups]
    ...
    plt.savefig("SquareBondPercolation_50x50_p49.png", dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    # Square Example
    width, height = (50, 50)
    # Generating bonds with open probability = 1/2:
    square_bond_bools = np.random.uniform(0.0, 1.0, (2, width, height)) < 0.49
    # Enforce boundary conditions, where bond 0 is right and bond 1 down
    square_bond_bools[0, -1, :] = False
    square_bond_bools[1, :, -1] = False

    # Calculating which sites are connected or in groups together


    draw_square_bonds(square_bond_bools, False, graph_params=(4, 10, 5))
